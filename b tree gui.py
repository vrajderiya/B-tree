...
class BST:
    def __init__(self, root):
        ...
        self.make_canvas = Canvas(self.window,width=1200,height=690,bg="chocolate",relief=RAISED,bd=8)
        self.make_canvas.place(x=0,y=0)
        # Canvas make for result presentation
        self.result_canvas = Canvas(self.window,width=1500,height=168,bg="#FF8C00",relief=RAISED,bd=4)
        self.result_canvas.place(x=0,y=690)
    ...
    def heading_root_and_null_make(self):# By default window presentation
        make_heading = Label(self.make_canvas,text="B Tree Search Visualizer",font=("Arial",27,"bold","italic"),fg="yellow",bg="chocolate")
        make_heading.place(x=475,y=10)
        make_root = Label(self.make_canvas,text="Root",font=("Arial",17,"bold"),fg="brown",bg="chocolate")
        make_root.place(x=575,y=10+50)
        ...
        self.make_null.place(x=575,y=90+50)
    ...
    def make_instructional_buttons(self):# For make Instructional buttons
        ...
        self.insert_node.place(x=1250,y=250)

        # Delete Button
        self.make_delete = Button(self.window, text="Delete", font=("Arial",20, "bold"), bg="black", fg="red", relief=RAISED, bd=8, command=lambda: self.take_input_set_up(2), state=NORMAL)
        self.make_delete.place(x=1250,y=325)

          # Search Button
        self.search_node = Button(self.window, text="Search", font=("Arial",20, "bold"), bg="black", fg="blue", relief=RAISED, bd=8, command=lambda: self.take_input_set_up(3), state=NORMAL)
        self.search_node.place(x=1250,y=410)

    def take_input_set_up(self,indicator):# Set up for take input value for node insert and delete
        self.input_btn_deactivation()
        self.take_input = Label(self.window, text="Enter the number to input", fg="blue", bg="orange", font=("Arial", 14, "bold"))
        self.take_input.place(x=1225, y=50)
        self.take_entry = Entry(self.window, fg="blue", bg="white", relief=SUNKEN, bd=6, width=5, font=("Arial", 15, "bold"))
        self.take_entry.place(x=1250, y=85)
        self.take_entry.focus()
        self.add_btn = Button(self.window, text="Add", fg="green", bg="black", relief=RAISED, bd=6, width=5, font=("Arial", 20, "bold"), state=NORMAL)
        self.add_btn.place(x=1250, y=135)
        ...
        if indicator == 2:# For delete node
        ...
        if indicator == 3:# For search node
           self.take_input['text'] = "Enter the number to search"
           self.add_btn['text'] = "Search"
           self.window.unbind('<space>')
           self.add_btn['command'] = lambda: self.filtration_of_input_value(False,indicator)
           self.window.bind('<Return>', lambda e: self.filtration_of_input_value(e, indicator))
            ...
            if int(self.take_entry.get()):
                    ...
                    if indicator == 2:
                        if self.node_number_value_store:# Delete available for only non empty Tree
                           ...
                           messagebox.showerror("Empty B Tree", "Nothing to delete is B Tree")
                    if indicator == 3:
    ...
    def make_default_node_with_set_position(self):# For node make and set at right position in canvas
        try:
            ...
            self.status_note.place(x=525, y=50)
    ...
    def reset_and_store(self):# Reset and store data by default
        ...
        self.status_note.place(x=550, y=50)
    ...
    def make_container_in_result_canvas(self):# In down canvas
        ...
        make_status.place(x=580,y=20)

        self.status_note = Label(self.result_canvas,bg="#FF8C00",fg="blue",text="Lets Start with B Tree",font=("Arial",20,"bold","italic"))
        self.status_note.place(x=520,y=50)       
    ...
    def delete(self):# For node deletion
        ...
        if len(self.node_number_value_store) == 0:
            messagebox.showwarning("Empty B Tree", "B Tree is empty..nothing to delete")
        else:
    ...
    def search(self):# For node searching
        val = self.take_entry.get()
        p_node = None

        if len(self.node_number_value_store) == 0:
            messagebox.showwarning("Empty B Tree", "B Tree is empty..nothing to search")
        else:
            node = self.node_number_value_store[0]# Consider first node by default to find targeting node
            while True:
                if node is None:
            ...
            if node is not None:
                if node[5] is None and node[6] is None:# No child node exist
                    self.search_no_child_node_exist(node,p_node)
                elif node[5] is None and node[6] is not None:# No left child node but right child node exist
                    self.search_no_child_node_exist(node,p_node)
    ...
    def search_no_child_node_exist(self,node,p_node):
        if p_node is None:
            pass
        else:
            if p_node[5] is node:
        ...
        self.make_canvas.itemconfig(node[0], fill="cyan", outline="black")
        node[1].config(bg="cyan", fg="red")
        self.status_note.config(text="The input node to be searched is in cyan colour")
        self.status_note.place(x=490, y=50)
        self.window.update()
        time.sleep(2)
        print(self.node_number_value_store)
        self.status_note.config(text="Work done")
    ...
    def color_indicator_to_delete(self,node,temp):# Color indicating to deleted and replaced node
        ...
        self.status_note.place(x=250, y=50)
        self.window.update()
        time.sleep(3)
